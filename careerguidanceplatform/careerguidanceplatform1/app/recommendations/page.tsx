      ...
      <div className="container mx-auto px-2 sm:px-4 py-4 sm:py-8 pt-20 sm:pt-24">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="text-center mb-4 sm:mb-8">
            ...
            <h1 className="text-2xl sm:text-4xl font-serif font-bold text-foreground mb-2 sm:mb-4">
            ...
            <Badge variant="secondary" className="mt-2 sm:mt-4 text-xs sm:text-sm">
              Profile Type: {recommendationType.charAt(0).toUpperCase() + recommendationType.slice(1)} Focused
            </Badge>
          </div>

          <Tabs defaultValue="careers" className="space-y-4 sm:space-y-8">
            ...
            <TabsContent value="careers" className="space-y-3 sm:space-y-6">
                      ...
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 mb-2">
                            <CardTitle className="text-lg sm:text-2xl font-serif">{career.title}</CardTitle>
                            <Badge variant="secondary" className="bg-accent/10 text-accent text-xs sm:text-sm w-fit">
                        ...
                        <Button
                          ...
                          className={`ml-2 h-8 w-8 sm:h-9 sm:w-auto ${savedItems.includes(`career-${index}`) ? "bg-accent/10" : ""}`}
                        >
                    ...
                    <CardContent className="pt-0">
                      <div className="space-y-4 sm:grid sm:grid-cols-2 sm:gap-6 sm:space-y-0">
                        ...
                        <div className="space-y-2 sm:space-y-3">
                          <div>
                            <span className="font-semibold text-xs sm:text-sm">Average Salary:</span>
                            <p className="text-muted-foreground text-xs sm:text-sm">{career.averageSalary}</p>
                          </div>
                          <div>
                            <span className="font-semibold text-xs sm:text-sm">Job Growth:</span>
                            <p className="text-muted-foreground text-xs sm:text-sm">{career.growthRate}</p>
                          </div>
                          <div>
                            <span className="font-semibold text-xs sm:text-sm">Education:</span>
                            <p className="text-muted-foreground text-xs sm:text-sm">{career.education}</p>
                          </div>
                        </div>
                      </div>
                    </CardContent>
            ...
            <TabsContent value="courses" className="space-y-3 sm:space-y-6">
                ...
                {courses[recommendationType].map((course, index) => (
                  <Card key={index} className="hover:shadow-lg transition-shadow">
                    <CardHeader className="pb-2 sm:pb-6">
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          <CardTitle className="text-base sm:text-lg font-serif mb-1 sm:mb-2">{course.title}</CardTitle>
                          <CardDescription className="text-xs sm:text-sm">{course.provider}</CardDescription>
                        </div>
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => handleSaveItem(`course-${index}`)}
                          className={`ml-2 h-8 w-8 ${savedItems.includes(`course-${index}`) ? "bg-accent/10" : ""}`}
                        >
                    ...
                    <CardContent className="space-y-2 sm:space-y-3 pt-0">
                      ...
                      <Button
                        variant="outline"
                        className="w-full mt-2 sm:mt-4 bg-transparent h-8 sm:h-9 text-xs sm:text-sm"
                      >
            ...
            <TabsContent value="certifications" className="space-y-3 sm:space-y-6">
              <div className="grid gap-3 sm:gap-6 sm:grid-cols-2 lg:grid-cols-3">
                {certifications[recommendationType].map((cert, index) => (
                  <Card key={index} className="hover:shadow-lg transition-shadow">
                    <CardHeader className="pb-2 sm:pb-6">
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          <CardTitle className="text-base sm:text-lg font-serif mb-1 sm:mb-2">{cert.title}</CardTitle>
                          <CardDescription className="text-xs sm:text-sm">{cert.provider}</CardDescription>
                        </div>
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => handleSaveItem(`cert-${index}`)}
                          className={`ml-2 h-8 w-8 ${savedItems.includes(`cert-${index}`) ? "bg-accent/10" : ""}`}
                        >
                          <Save className="h-3 w-3" />
                        </Button>
                      </div>
          ...
          <div className="text-center mt-4 sm:mt-8">
            <Button onClick={handleSaveToProfile} size="lg" className="h-10 sm:h-11 text-sm sm:text-base">
